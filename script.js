...
async function setupProductPage(){
  ...
  // Render thumbnails
  productImagesEl.innerHTML = images.map(url=>`<img src="${url}" class="w-48 h-48 object-contain border rounded cursor-pointer hover:scale-105 transition" />`).join("");

  // Select image
  let selectedImage = images[0] || "";
  productImagesEl.querySelectorAll("img").forEach(imgEl=>{
    imgEl.addEventListener("click", ()=> selectedImage = imgEl.src);
  });
